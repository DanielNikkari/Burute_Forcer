...
def main():
    ...
    wordlist_container = wordlists("/Users/danielnikkari/Desktop/Aalto/Security_Engineering/Ex2/Python/Burute_Forcer/my_wordlist/")
    ...
    try:
        for attempt in bruteforce(ascii_charset_letters, minlength, maxlength):
            #stdout.write("\r%d " % n)
            for word in wordlist_container:
                word = word.strip()
                stdout.write("\r{} {}{}".format(n, word, attempt))
                stdout.flush()
                word_attempt = word + attempt
                # Transform the attempt into a hash
                attempt_hash = hashlib.sha1(bytes(word_attempt, 'ascii')).hexdigest()
                # print(attempt)
